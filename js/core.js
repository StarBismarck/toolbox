...
window.addEventListener("online",function(){toast("🟢 网络已恢复")});window.addEventListener("offline",function(){toast("🔴 网络已断开，数据将保存到本地")});var Store={_k:function(k){return'tb_'+(App.user||'')+'_'+k;},
  ...
  _sync:function(k,v){if(!App.user)return;var u=App.user,b={username:u,data_type:k,data_json:JSON.stringify(v),updated_at:new Date().toISOString()};API.upsert('toolbar_data','username=eq.'+encodeURIComponent(u)+'&data_type=eq.'+encodeURIComponent(k),b).catch(function(e){ErrorLog.log("sync",e.message)});},
  syncUsers:function(){API.upsert('toolbar_data','username=eq._system&data_type=eq._users',{username:'_system',data_type:'_users',data_json:JSON.stringify(Auth.users()),updated_at:new Date().toISOString()}).catch(function(e){ErrorLog.log("sync",e.message)});},
...
var Stats={track:function(tool,action){try{var now=new Date().toISOString();API.upsert("toolbar_data","username=eq._stats&data_type=eq.usage",{username:"_stats",data_type:"usage",data_json:JSON.stringify({tool:tool,action:action,user:App.user||"?",time:now}),updated_at:now}).catch(function(){})}catch(e){}}};var ErrorLog={log:function(src,msg){try{var k="err_"+src+"_"+Date.now();API.upsert("toolbar_data","username=eq._errors&data_type=eq."+encodeURIComponent(k),{username:"_errors",data_type:k,data_json:JSON.stringify({source:src,message:msg,time:new Date().toISOString(),user:App.user||"anonymous"}),updated_at:new Date().toISOString()}).catch(function(){})}catch(e){}}};